...
observed_ref <- tb %>% 
    ...
    select(taxa) %>%
    as("matrix")
corner(observed_tar, 5, 4)
corner(observed_ref, 5, 4)
## Screen out taxa 
observed_tar <- observed_tar * screen_mat
observed_ref <- observed_ref * screen_mat
corner(observed_tar, 5, 4)
corner(observed_ref, 5, 4)
```
...
Debugging
```{r}
# prevotella <- str_subset(colnames(observed_tar), "Prevotella")
# observed_tar[,prevotella]
# observed_ref[,prevotella]
```
...
Reduced subset of taxa for plotting:
```{r}
...
two_taxa <- tax %>% 
    ...
    sample_n(1) %$%
    Taxon
# Prune to the lower half (+ the mock) and just the taxa with bias estimates
tree <- phy_tree(ps0)
mrca_node <- ape::getMRCA(tree, two_taxa)
taxa <- tree %>%
...
```{r}
tb <- td %>%
    filter(taxa %in% mock_taxa)
mock_mean <- mean(tb$mean)
tb %>%
    mutate_at(2:7, ~ . - mock_mean)
