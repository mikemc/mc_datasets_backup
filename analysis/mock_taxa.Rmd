...
spikein_density <- truth %>% filter(Species == spikein) %$% 2**Log2_density
# get the AAs 
...
```{r}
tb <- list(ps1_eval_aa, ps1_eval_cal_aa) %>% 
    map(as_tibble) %>%
...
cdiff_density <- truth %>% filter(Species == "Clostridium_difficile") %$%
    2**Log2_density
ggplot(tb, aes(x = Clostridium_difficile, y = Protocol, color = Calibrated)) +
