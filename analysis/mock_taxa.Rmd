...
Another way to see this information along with the variation in expected
frequency:
```{r}
...
```{r}
ggplot(tb, 
    aes(x = factor(Taxon, species_ordered), y = CLR, color = Protocol, 
        label = Individual)) + 
    geom_point(data = mock_comp, 
        mapping = aes(x = factor(Taxon, species_ordered), y = CLR), 
...
For the poster, I should change the y-axes to be in terms of the ratio rather
than the log.
...
```{r}
# Add the truth
st_truth <- mock_comp %>% select(CLR) %>% as("matrix") %>% t
colnames(st_truth) <- mock_comp$Taxon
...
```{r}
# ps1_eval_merged <- merge_phyloseq(ps1_eval_clr, ps1_eval_clr_cal, 
ps2 <- ps1_eval_clr
ps2_cal <- ps1_eval_clr_cal
sample_names(ps2_cal) <- sample_names(ps2_cal) %>% paste0(".cal")
sample_data(ps2)$Calibrated <- "No"
sample_data(ps2_cal)$Calibrated <- "Yes"
sample_data(ps_truth_clr)$Calibrated <- "Truth"
ps2_all <- merge_phyloseq(ps2, ps2_cal, ps_truth_clr)
```

NMDS:
```{r}
ord <- ordinate(ps2_all, "NMDS", "euclidean")
```
```{r, fig.height = 6, fig.width = 8}
p <- plot_ordination(ps2_all, ord, type="samples", color="Protocol", 
    shape = "Calibrated")
ggplot(p$data, aes(x = NMDS1, y = NMDS2, 
...
```{r, eval=F}
# For plotting: Remove Truth as a protocol, but keep as a calibration status;
# give a copy for each protocol
ggplot(pd, aes(x = Sample, y = Abundance, fill = Taxon)) +
    geom_bar(stat = "identity") +
    facet_grid(. ~ Protocol, 
        scales = "free", space = "free", shrink = TRUE)
```
...
ps1_eval_cal <- transform_sample_counts(ps1_eval_clr_cal, 
...
# get the AAs 
ps1_eval_aa <- ps1_eval %>%
    transform_sample_counts(function (x) {x * spikein_density / x[spikein]})
ps1_eval_cal_aa <- ps1_eval_cal %>%
    transform_sample_counts(function (x) {x * spikein_density / x[spikein]})
```

```{r}
aa_tb <- list(ps1_eval_aa, ps1_eval_cal_aa) %>% 
    map(as_tibble) %>%
    bind_rows(.id = "Calibrated") %>%
    mutate(Calibrated = map_chr(Calibrated, switch, "1" = "No", "2" = "Yes"))
```

```{r, fig.height = 4, fig.width = 6}
cdiff_density <- mock_comp %>% filter(Taxon == "Clostridium_difficile") %$%
    2**Log2_density
ggplot(aa_tb, aes(x = Clostridium_difficile, y = Protocol, color = Calibrated)) +
