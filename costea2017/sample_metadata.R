...
## Get the SRA run identifiers, to connect samples to their sequence data
...
# The Library name can be used to connect back to the Sample name in the
# study metadata, and the SRA run identifier can be used to download the
# sequence data using NCBI's SRA toolkit.
tb <- tibble(
    Library = xml %>%
...
# Next, I want to add the SRA runs to the sample metadata table. But the sample
# names in the nucleotide servers and in the sample metadata can include some
# extra characters, causing them not to match up, and these differ for the
# three Phases.
